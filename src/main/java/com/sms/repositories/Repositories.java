...
public class Repositories {
	...
	public interface SmsMasterRepository extends JpaRepository<SmsMaster, Long> {
		...
		SmsMaster findTopByMobileNoOrderByOtpDateDesc(String mobileNo);
	...
	public interface UserMasterRepository extends JpaRepository<UserMaster, String> {
		...
		@Transactional
		@Modifying
		@Query(value = "UPDATE user_master SET passwd=:passwd, passwd_exp=:passwdExp, auth_status='1' WHERE user_id = :userId", nativeQuery = true)
		int updatePasswordMaster(@Param("passwd") String passwd, @Param("passwdExp") Date passwdExp,
				@Param("userId") String userId);

		@Transactional
		@Modifying
		@Query(value = "UPDATE user_master SET logout_status=:logoutStatus, last_login_dt=:lastLoginDt WHERE user_id = :userId", nativeQuery = true)
		int updateLoginDetailsMaster(@Param("logoutStatus") String logoutStatus, @Param("lastLoginDt") Date lastLoginDt,
				@Param("userId") String userId);

		@Transactional
		@Modifying
		@Query(value = "UPDATE user_master SET logout_status=:logoutStatus, last_logout_date=:lastLogoutDate WHERE user_id = :userId", nativeQuery = true)
		int updateLogOutDetailsMaster(@Param("logoutStatus") String logoutStatus,
				@Param("lastLogoutDate") Date lastLogoutDate, @Param("userId") String userId);

		@Transactional
		@Modifying
		@Query(value = "UPDATE user_master SET login_attempt=:loginAttempt, lock_time=:lockTime, last_login_dt=:lastLoginDt WHERE user_id = :userId", nativeQuery = true)
		int updateUnloackPeroid(@Param("loginAttempt") Integer loginAttempt, @Param("lockTime") Date lockTime,
				@Param("lastLoginDt") Date lastLoginDt, @Param("userId") String userId);

		@Transactional
		@Modifying
		@Query(value = "UPDATE user_master SET login_attempt=:loginAttempt, lock_time=:lockTime WHERE user_id = :userId", nativeQuery = true)
		int updateLockingPeroid(@Param("loginAttempt") Integer loginAttempt, @Param("lockTime") Date lockTime,
				@Param("userId") String userId);
	}
